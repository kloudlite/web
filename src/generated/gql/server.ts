...
export type K8s__Io___Api___Core___V1__Protocol = 'SCTP' | 'TCP' | 'UDP';
...
export type ConsoleListHelmChartsQueryVariables = Exact<{
  envName: Scalars['String']['input'];
}>;

export type ConsoleListHelmChartsQuery = {
  core_listHelmCharts?: {
    totalCount: number;
    pageInfo: {
      endCursor?: string;
...
export type ConsoleCreateHelmChartMutation = {
  core_createHelmChart?: { id: string };
};
...
export type AuthCli_UpdateEnvironmentMutation = {
  core_updateEnvironment?: { id: string };
};
...
export type AuthCli_GetByokClusterQuery = {
  infra_getBYOKCluster?: {
    clusterToken: string;
    displayName: string;
    lastOnlineAt?: any;
    markedForDeletion?: boolean;
    metadata: { labels?: any; name: string; namespace?: string };
  };
};
