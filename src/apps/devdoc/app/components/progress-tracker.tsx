    ...
    <div className="flex flex-col">
      {items.map((item, index) => (
        <div key={item.id} className="flex flex-row gap-xl">
          ...
          >
            <ProgressCircle />
            <div className="border-r border-dashed flex-1" />
          </div>
          <div
            className={cn(
              index !== items.length - 1 ? 'pb-3xl' : '',
              index !== 0 ? '-mt-[6px]' : ''
            )}
          >
            {item.render()}
