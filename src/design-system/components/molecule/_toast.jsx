      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn(
              'toast flex flex-row rounded shadow-popover text-text-on-primary p-xl gap-xl items-center justify-between border transition-all overflow-hidden',
              {
                'opacity-0': !open,
          ...
          >
            <ToastRadix.Title className="bodyMd-medium truncate capitalize">
              {/* {toastType} */}
              {content}
            </ToastRadix.Title>
...
export const ToastProvider = ({ children }) => {
  ...
  return (
    <ToastRadix.Provider swipeDirection="right" duration={0}>
      ...
      <ToastRadix.Viewport className="flex flex-col gap-lg fixed max-w-14xl  min-w-13xl top-8xl right-8xl m-0 list-none z-[2147483647] outline-none" />
    </ToastRadix.Provider>
  );
};
